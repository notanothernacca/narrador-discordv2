    ...
    async def _process_queue(self, guild: discord.Guild):
        ...
        try:
            ...
            while self.queue:
                ...
                voice_channel = None
                # Siempre usar el canal de voz definido en VOICE_CHANNEL_ID
                if self.config.VOICE_CHANNEL_ID:
                    voice_channel = discord.utils.get(
                        guild.voice_channels, id=int(self.config.VOICE_CHANNEL_ID)
                    )
                    if not voice_channel:
                        logger.warning(
                            f"No se encontró el canal de voz por defecto con ID {self.config.VOICE_CHANNEL_ID}"
                        )
                        self.queue.popleft()
                        continue
                else:
                    logger.warning(
                        "No hay canal de voz por defecto configurado (VOICE_CHANNEL_ID)"
                    )
                    self.queue.popleft()
                    continue
